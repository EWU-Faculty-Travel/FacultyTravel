...
class Triplist extends MY_Controller
{
	...
	public function action($page = 'Trip Listing')
    {
		...
		} else if(element('action', $form_data) == 'print') //PRINT AN ACCEPTED TRIP
		{
			...
			$boxSixTotal = $this->print_form->get_Total();
			$indexTableTotal = $this->print_form->get_IndexTotalAmount();
			
			if($boxSixTotal == $indexTableTotal)
			{
				/*Print code!*/
			...
			$this->load->view('templates/footer');
		} else if(element('action', $form_data) == 'index' && ($return_to == 2 || ($return_to == 1 && ($status == 0 || $status == 4)))) //EDIT THE INDEX CODES
        {
        	$this->print_form->setDBTripID($trip_id);
            $this->print_form->set_id($trip_id);
            $data['box_6_total'] = $this->print_form->get_Total();
            $this->print_form->deleteDBTripID();
            $data['title'] = 'Modify Index Codes';
    ...
    public function postAction($page = 'Trip Listing')
    {
        ...
        } else if(element('action', $form_data) == 'print' && ($status == 8)) //PRINT AN ACCEPTED POST TRIP
        {
